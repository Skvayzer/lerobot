...
#SBATCH --qos=gtqos
#SBATCH --mem=512G
#SBATCH --job-name=groot_dex1sim_v2
#SBATCH --output=/vast/users/chenyuan.chen/constantine/unitree_IL_lerobot/unitree_lerobot/lerobot/slurm_groot_dex1sim_v2_%j.log
#SBATCH --error=/vast/users/chenyuan.chen/constantine/unitree_IL_lerobot/unitree_lerobot/lerobot/slurm_groot_dex1sim_v2_%j.log
...
echo "=========================================="
echo "GR00T Dex1 Sim BlockStacking v2 (fixed normalization)"
echo "Key fix: pipeline.py now correctly applies dataset min/max stats"
echo "after loading safetensors, so training uses actual data ranges."
...
OUTPUT_DIR=outputs/train/$(date +%Y%m%d_%H%M%S)_groot_dex1sim_blockstack_v2
...
accelerate launch \
  ...
  --wandb.notes="GR00T Dex1 Sim v2: fixed pipeline.py normalization override (dataset stats now correctly applied after safetensors load)"
